...
export function EventForm({ team, event, onDone, onCancel, onDenied }: {
  ...
  const [modality, setModality] = useState(event?.modality || team.modalities[0]);
  ...
  async function save() {
    ...
    const start = isoDate(date), end = isoDate(until);
    if (!title.trim() || !location.trim() || !modality) { setError('Informe título, modalidade e local.'); return; }
    if (!start || !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) { setError('Confira a data (DD/MM/AAAA) e o horário (HH:MM).'); return; }
    ...
    try {
      onDone(await saveEvent(team.id, { title: title.trim(), kind, modality, date: start, time,
  ...
  return <View style={styles.stack}>
    ...
    <View style={styles.row}>{options?.modalities.filter(item => team.modalities.includes(item.value)).map(item => <Pressable key={item.value} disabled={busy} accessibilityRole="radio" accessibilityLabel={item.label} accessibilityState={{ checked: modality === item.value }} aria-checked={modality === item.value}
      onPress={() => setModality(item.value)} style={[styles.chip, modality === item.value && styles.selected]}><Text style={styles.text}>{item.label}</Text></Pressable>)}</View>
